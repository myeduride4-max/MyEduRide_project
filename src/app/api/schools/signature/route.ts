...
import { resolveLogoMime, uploadSchoolSignatureBuffer } from '@/lib/storage/upload-school-logo';

export const dynamic = 'force-dynamic';

function canEditSignature(
  session: NonNullable<ReturnType<typeof getSessionFromRequest>>,
  schoolId: string
): boolean {
  if (sessionHasRole(session, 'super_admin')) return true;
  return session.roles.some((r) => r.role === 'school_admin' && r.school_id === schoolId);
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!canEditSignature(session, schoolId)) {
    ...
    if (!mime) {
      return NextResponse.json(
        { error: 'Signature must be JPG, PNG, or WebP (max 5 MB).' },
        { status: 400 }
      );
    }
    ...
    const { path, error: uploadErr } = await uploadSchoolSignatureBuffer(
      supabase,
      schoolId,
      buffer,
      mime,
    ...
    const { error: dbErr } = await supabase
      .from('schools')
      .update({ principal_signature_url: path })
      .eq('id', schoolId);
